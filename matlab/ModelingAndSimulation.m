...
for attempt = 1:MaxAttempts
    ...
    fprintf("Launch fuel ratio = %f\n", launch_ratio);
    
    % Solve the takeoff angle
    fprintf("1) Solving takeoff angle......\n");
    [takeoff_angle, success] = SolveTakeoffAngle(launch_ratio, data);
    I_apex = data.I;
    if success == 0
        fprintf("--- Takeoff angle search failed.  Increasing launch ratio.\n");
    ...
    fprintf("--- Landed with %fkg of fuel at %fm/s.\n", remainingMass, landingSpeed);
    
    % Is this optimal?
    if data.vx(I_apex) >= MaxDiscoveredVx && landingSpeed < LandingVelocityAllowance
        fprintf("*** This is a new optimal spec!\n");
    ...
    if remainingMass > 0 && abs(landingSpeed) < LandingVelocityAllowance
        fprintf("Increasing launch fuel ratio.\n");
        if ratioAdjustmentDir == -1
            ratioAdjustment = ratioAdjustment/2;
        end
        launch_ratio = launch_ratio + ratioAdjustment;
        ...
        fprintf("Reducing launch fuel ratio due to land failure.\n");
        if ratioAdjustmentDir == 1
            ratioAdjustment = ratioAdjustment/2;
        end
        launch_ratio = launch_ratio - ratioAdjustment;
...
fprintf("##########\nAlgorithm complete!\n");
fprintf("Best launch ratio = %f\n", launch_ratio_BEST);
fprintf("Required takeoff angle = %f\n", takeoff_angle_BEST);
fprintf("Achieved horizontal velocity = %f m/s\n", MaxDiscoveredVx);
fprintf("Landing Burn Altitude = %f\n", landingBurnAltitude_BEST);


%% Plots
% First need to trim the results
t = data.t(1:I_land_BEST);
z = data.z(1:I_land_BEST);
vz = data.vz(1:I_land_BEST);
az = data.az(1:I_land_BEST);
x = data.x(1:I_land_BEST);
vx = data.vx(1:I_land_BEST);
Fd = data.Fd(1:I_land_BEST);
...
function [pe, Ae, mdot] = SolveThrust(vjet, T_sea, T_vac, Isp_sea, Isp_vac, N)
    % Vacuum first
...
function [takeoff_angle, success] = SolveTakeoffAngle(launch_ratio, data)
    % Bring in globals
    ...
    while 1 % Keep looping until terminated
        % Initial conditions
        data.theta(1) = 0;
        ...
        if error <= TargetAltitude * errorAllowance
            success = 1;
            fprintf("--- Error = %fm after %d attempts.\n", error, attempt);
            return;
        end
...
function landingBurnAltitude = SolveLandingBurn(data, I_apex)
    ...
    for attempt = 1:40
        ...
        while data.z(data.I-1) > 0
            ...
            % Initiate burn?
            if burnInitiated_I == 0 && data.z(data.I-1) <= landingBurnAltitude
                burnInitiated_I = data.I;
            end
            
            % Done with burn?
            if burnInitiated_I > 0 && burnStopped_I == 0
                % Out of fuel?
                if data.m(data.I-1) <= stopMass
                    burnStopped_I = data.I;
                    outOfFuel = 1;
                elseif data.velocity(-1) < LandingVelocityAllowance/4
                    burnStopped_I = data.I;
        ...
        if landedSafely
            fprintf("--- Landed safely after #%d attempts.\n", attempt);
            fprintf("--- Landing burn altitude = %dm\n", landingBurnAltitude);
            return;
        end
        ...
        elseif outOfFuel
            ...
            if data.z(burnStopped_I) <= 500
                fprintf("--- Ran out of fuel at low altitude.\n");
                return;
            end
    ...
    fprintf("--- Number of attempts has expired.\n");
end
